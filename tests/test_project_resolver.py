...
class ProjectResolverTests(unittest.TestCase):
    ...
    def test_git_repository_root_subdirectory_and_worktree_share_one_slug(self):
        ...
        for start in (repo, repo / "src" / "pkg", worktree, worktree / "lib"):
            with self.subTest(start=start):
                ...
                assert identity.project == "my-cool-app"
                assert identity.source == "git"
        assert resolve_project(worktree).worktree == str(worktree)
        assert resolve_project(repo / "src").worktree == str(repo)
    ...
    def test_package_marker_without_git(self):
        ...
        assert (identity.project, identity.source) == ("svc", "marker")
    ...
    def test_pin_file_beats_directory_name(self):
        ...
        assert (identity.project, identity.source) == ("intuicryp-hermes", "pin")
    ...
    def test_vault_override_beats_everything_and_prefers_longest_prefix(self):
        ...
        assert resolve_project(inner, vault=vault).project == "inner-name"
        assert resolve_project(repo / "src", vault=vault).project == "outer-name"
        assert resolve_project(repo / "src", vault=vault).source == "override"
        # A corrupt override file must not break resolution.
        (vault / ".ai-memory-hub" / "projects.json").write_text("{not json", encoding="utf-8")
        assert resolve_project(inner, vault=vault).project == "nested"

    def test_explicit_project_is_honoured(self):
        repo = self._repo("whatever")
        identity = resolve_project(repo, explicit="Trading Agent")
        assert (identity.project, identity.source) == ("trading-agent", "explicit")
    ...
    def test_non_project_directories_are_unscoped_and_never_merged(self):
        for path in (Path.home(), Path(os.environ.get("SystemRoot", "C:/Windows")) / "System32"
            ...
            with self.subTest(path=path):
                ...
                assert identity.project == UNSCOPED
        assert resolve_project(None).project == UNSCOPED
        assert resolve_project("").project == UNSCOPED

    def test_vanished_directory_still_resolves_deterministically(self):
        identity = resolve_project(self.root / "gone" / "ghost-project")
        assert (identity.project, identity.source) == ("ghost-project", "cwd")
    ...
    def test_cached_variant_returns_identical_result(self):
        ...
        assert resolve_project_cached(repo / "src") == resolve_project(repo / "src")
        assert isinstance(resolve_project_cached(repo), ProjectIdentity)
    ...
    def test_no_git_or_network_is_required(self):
        ...
        try:
            os.environ["PATH"] = ""
            assert resolve_project(repo).project == "offline"
        finally:
            if old is not None:
                os.environ["PATH"] = old
