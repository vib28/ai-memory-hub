...
SETTINGS_SCHEMA: list[dict[str, Any]] = [
    ...
    {"key": "MEMORY_HANDOFF_MAX_CHARS", "group": "worker", "label": "Max startup context packet size (chars)", "type": "int",
     "default": 6000, "min": 500, "max": 12000},
    {"key": "MEMORY_TURN_MAX_CHARS", "group": "worker", "label": "Max per-prompt context packet size (chars)", "type": "int",
     "default": 1500, "min": 200, "max": 12000,
     "description": "Delta-only packet injected on each user prompt; silent when nothing new is relevant."},
    {"key": "MEMORY_HANDOFF_CATCHUP_SECONDS", "group": "worker", "label": "SessionStart catch-up budget (s)", "type": "int",
     "default": 2, "min": 0, "max": 30,
     "description": "Time the startup hook may spend checkpointing this project's still-pending sessions before it reads the manifest. 0 disables the pass."},
    {"key": "MEMORY_INLINE_CONSOLIDATION", "group": "worker", "label": "Spawn a detached checkpoint on session-end/stop/compaction", "type": "bool",
     "default": True,
     "description": "The hook receiver starts a one-shot worker child on terminal events so checkpoints exist without the resident worker. Turn off only if you run the resident worker and want it to own all consolidation."},
