...
async function refresh(){const resp=await api('/api/memories');state.rows=Array.isArray(resp)?resp:resp.rows||[];$('total').textContent=resp.total||state.rows.length;options('kind',[...new Set(state.rows.map(r=>r.kind))].sort(),'All types');options('tag-filter',[...new Set(state.rows.flatMap(r=>r.tags||[]))].sort(),'All tags');renderList();if(state.selected&&state.view==='library')await select(state.selected);const pending=await api('/api/pending');$('pending-count').textContent=pending.length||'';const worker=await api('/api/worker-health');$('worker-status').textContent='Worker: '+human(worker.status||'unknown')+(worker.backlog?' · '+worker.backlog+' queued':'');}
...
if(view==='audit'){const result=await api('/api/audit');const issues=await api('/api/audit/issues');if(state.view!==view)return;host.innerHTML='<h2>'+ (result.healthy?'Your vault checks passed.':'Your vault needs attention.')+'</h2><p class="muted">Checks stored files against the search index. This does not change your memories.</p>';if(result.healthy&&!issues.issues.length){host.insertAdjacentHTML('beforeend','<h2>All clear — no fixable issues.</h2>');return;}
const section=document.createElement('section');section.className='audit-issues';const heading=document.createElement('h2');heading.textContent='Fixable issues ('+issues.issues.length+')';section.append(heading);for(const issue of issues.issues){const card=document.createElement('article');card.className='review-card audit-issue';const meta=document.createElement('div');meta.className='meta';meta.textContent=issue.severity+' · '+issue.issue_type+(issue.path?' · '+issue.path+(issue.line?' #'+issue.line:''):'');const summary=document.createElement('h3');summary.textContent=issue.summary;card.append(meta,summary);if(issue.text){const evidence=document.createElement('pre');evidence.textContent=issue.text.slice(0,300);card.append(evidence);}if(issue.fixable){const footer=document.createElement('footer');const fixBtn=button('Auto-Fix',async()=>{fixBtn.disabled=true;try{const res=await api('/api/fix-issue',issue);notice('Fix applied: '+res.fix.message);await showView('audit');}catch(e){notice('Fix failed: '+e.message,true);fixBtn.disabled=false;}},'primary');footer.append(fixBtn);card.append(footer);}section.append(card);}host.append(section);}}
