...
Still open: [#13](https://github.com/vib28/ai-memory-hub/issues/13) (roadmap/closeout) and [#27](https://github.com/vib28/ai-memory-hub/issues/27) (remaining worst-case write-path duplicate-scan performance).
...
Tiers 0–4 are complete on `enhancements/roadmap`. The observation buffer, local consolidation, optional hybrid retrieval, session import, bounded context priming, and hook install/uninstall are implemented — [#14](https://github.com/vib28/ai-memory-hub/issues/14) and [#29](https://github.com/vib28/ai-memory-hub/issues/29) are closed, so those safety gates are cleared. What remains is the worst-case write-path duplicate-scan performance work ([#27](https://github.com/vib28/ai-memory-hub/issues/27)); safe length-bound pruning and its benchmark are already landed, while embeddings still affect search/audit only, never write decisions.
